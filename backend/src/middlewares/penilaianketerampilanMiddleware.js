...
// Middleware khusus untuk penilaian keterampilan (hanya kepala unit dan mitra bestari)
export const authorizePenilai = (req, res, next) => {
  const allowedRoles = ["kepala unit", "mitra bestari"];
  if (!allowedRoles.includes(req.user.role)) {
    return res.status(403).json({
      success: false,
      message: "Access denied: Hanya kepala unit dan mitra bestari yang dapat mengakses penilaian keterampilan"
    });
  }
  next();
};